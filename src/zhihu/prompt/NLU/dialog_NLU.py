...
import json
import logging
from zhihu.common.api import get_completion, reset_session_message, get_session_messages, add_message_to_session
...
class NLU:
    ...
    def get_completion(self, prompt, model="gpt-3.5-turbo"):
        ...
        rsp = get_completion(prompt, model=model, temperature=0)
...
class DST:
    ...
    - 目前DST update只是做一些数据的清洗/转换工作；i.e. 如果name出现在从NLU传过来的json中，则清除原有state中的信息(可能意味着换了一个套餐); 根据sort信息清洗，etc
    ...
    def update(self, state, nlu_json):
        def _clear():
            if "name" in state:
                state.clear()
            if "sort" in nlu_json:
                slot = nlu_json["sort"]["value"] # the sorted field, e.g. month_data, month_price
                if slot in state:
                    del state[slot]
        _clear()
...
class MockedDB:
    ...
    def __init__(self):
        ...
        self.data = [
            {"name": "经济套餐", "month_price": 50, "month_data": 10, "requirement": None},
            {"name": "畅游套餐", "month_price": 180, "month_data": 100, "requirement": None},
            {"name": "无限套餐", "month_price": 300, "month_data": 1000, "requirement": None},
            {"name": "校园套餐", "month_price": 150, "month_data": 200, "requirement": "在校生"},
        ]
    ...
    def retrieve(self, **kwargs):
        ...
        def _sort_records():
            ...
            if "sort" in kwargs:
                ...
                reversed = kwargs["sort"]["ordering"] == "descend"
            return sorted(records, key=lambda x: x[key], reverse=reversed)
...
class DialogManager:
    ...
    def get_completion(self, prompt, model="gpt-3.5-turbo"):
        ...
        reset_session_message()
        rsp = get_completion(prompt, model=model, messages=session, temperature=0)
...
# (注意, 替换recommand prompt template的 _wrap()时候，已经得到答案了, 第2轮对话甚至是可省略的)
