""" Test API for Baidu ERNIE models """
import logging
...
def test_chat_do_with_multimessage():
    """ Test chat with ERNIE model with multi messages """
    session = Session()
    session.set_system_prompt("你是一个地理通")
    # temperature can't be 0 in ERNIE model
    rsp = session.chat("北京有7环路吗?", model="ERNIE-4.0-8K",
                       temperture=0.7, clear_session=False)
...
def test_chaput_do_with_only_message_param():
    """ Test chat with ERNIE model with only providing message param"""
